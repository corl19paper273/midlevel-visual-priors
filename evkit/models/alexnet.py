...
from evkit.models.architectures import FrameStacked, Flatten, atari_conv, init_
...
class AlexNet(nn.Module):
    def __init__(self, num_classes=1000):
        ...
        self.eval()
        self.eval_only=True
    def forward(self, x, stop_layer=19):
        if stop_layer == 'conv5':
            stop_layer = 11
        elif stop_layer == 'fc7':
            stop_layer = 19
...
class AlexNetFeaturesOnlyNet(nn.Module):
    ...
    def __init__(self, n_frames, n_map_channels=0, use_target=True,
        ...
        if self.n_map_channels > 0:
            self.map_tower = atari_conv(self.n_frames * self.n_map_channels)
        if self.use_target:
