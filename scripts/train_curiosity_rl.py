...
from evkit.models.actor_critic_module_curiosity import ForwardInverseACModule
from evkit.models.forward_inverse import *
...
from evkit.rl.storage import RolloutSensorDictStorage, RolloutSensorDictReplayBuffer, StackedSensorDictStorage, RolloutSensorDictCuriosityReplayBuffer
...
def run_training(cfg, uuid, override={}):
    try:
        ...
        envs = EnvFactory.vectorized(
                        ...
                        preprocessing_fn     = eval(cfg['env']['transform_fn_pre_aggregation'].replace("---", "'")),
        ...
        if 'transform_fn_post_aggregation' in cfg['env'] and cfg['env']['transform_fn_post_aggregation'] is not None:
            transform, space = eval(cfg['env']['transform_fn_post_aggregation'].replace("---", "'"))(envs.observation_space)
        ...
        if agent == None:
            ...
            forward = ForwardModel(cfg['learner']['internal_state_size'], (-1, 3), cfg['learner']['internal_state_size'])
            inverse = InverseModel(cfg['learner']['internal_state_size'], cfg['learner']['internal_state_size'], 3)
            ...
            if cfg['learner']['use_replay']:
                agent = evkit.rl.algo.PPOReplayCuriosity(actor_critic,
                                 ...
                                 curiosity_reward_coef=cfg['learner']['curiosity_reward_coef'],
                                 forward_loss_coef=cfg['learner']['forward_loss_coef'],
                                 inverse_loss_coef=cfg['learner']['inverse_loss_coef'])
            else:
                agent = evkit.rl.algo.PPOCuriosity(actor_critic,
                                  cfg['learner']['clip_param'],
                                  cfg['learner']['ppo_epoch'],
                                  cfg['learner']['num_mini_batch'],
                                  cfg['learner']['value_loss_coef'],
        ...
        for j in range(start_epoch, num_updates, 1):
            ...
            if (j) % cfg['saving']['log_interval'] == 0:
                ...
                n_steps_since_logging = cfg['saving']['log_interval'] * num_relevant_processes * cfg['learner']['num_steps']
